...
- Node 14 (edge review) is retired with Pass C — cross-tradition PARALLELS are
  now derived at node 16 (`docs/ingest/16-derive-parallels.md`), off reviewed
  tags, with no review queue. `/guru-review-edges` now only matters for the
  historical `staged_edges` queue, not new proposals.
