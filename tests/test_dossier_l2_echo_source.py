...
The fix threads the work's raw chunk bodies (``_work_chunk_bodies``) as the echo
source through every L2 summary-of-summaries stage, so reusing an input-summary
clause is allowed while a raw-passage echo is still caught.
"""
...
def _wp():
    spans = [{"text_id": "t", "slug": f"s{i}", "label": f"S{i}",
              "chunk_ids": [f"x.t.00{i}"], "token_count": 100} for i in (1, 2)]
...
def _prose(*, contains: str, filler: int = 100) -> str:
    ...
    guard's stride of 7 — so the old join-based guard was guaranteed to sample
    and trip on it), then distinct filler that appears in no source."""
...
def test_l2_echo_source_is_the_chunk_bodies_not_the_summary_join(db, monkeypatch):
    ...
    ground = "alpha beta gamma delta epsilon " * 20
    monkeypatch.setattr(gd, "_work_chunk_bodies", lambda rows: ground)
    ...
    assert seen_sources == [ground]                 # echo source is the ground
...
def test_l2_synthesis_reusing_an_l1_clause_is_accepted(db, monkeypatch):
    ...
    monkeypatch.setattr(gd, "count_tokens", _tok)
    monkeypatch.setattr(gd, "L2_INPUT_BUDGET", 16_000)
    monkeypatch.setattr(gd, "_work_chunk_bodies",
                        lambda rows: "alpha beta gamma delta epsilon " * 40)
...
def test_l2_synthesis_echoing_a_raw_chunk_passage_is_still_rejected(db, monkeypatch):
    ...
    monkeypatch.setattr(gd, "L2_INPUT_BUDGET", 16_000)
    monkeypatch.setattr(gd, "_work_chunk_bodies",
                        lambda rows: "Chunk body opens. " + RAW_PASSAGE + " end.")
